...
class VersionCompatibilityService {
  ...
  Future<void> initialize() async {
    try {
      ...
      _logger.log('客户端版本: $_clientVersion, 最小服务器版本: $_minServerVersion', tag: 'VERSION_COMPAT');
    } catch (e, stackTrace) {
  ...
  Future<(bool, String?)> checkServerVersion(String serverVersion) async {
    try {
      ...
      _logger.log('检查服务器版本兼容性: 服务器=$serverVersionNumber, 要求最小版本=$_minServerVersion', tag: 'VERSION_COMPAT');

      final isCompatible = _compareVersions(serverVersionNumber, _minServerVersion!);
      
      if (!isCompatible) {
        final reason = '服务器版本 $serverVersionNumber 低于客户端要求的最小版本 $_minServerVersion';
  ...
  /// 从VERSION.yaml文件读取最小服务器版本要求
  /// 支持YAML格式和旧格式（向后兼容）
  Future<String?> _readMinServerVersion() async {
    try {
      // 尝试从应用文档目录向上查找项目根目录的VERSION.yaml文件
            ...
            if (yamlDoc is Map) {
              ...
              if (compatibility is Map) {
                ...
                if (minVersion != null && minVersion.toString().isNotEmpty) {
                  ...
                  _logger.log('从VERSION.yaml文件读取最小服务器版本: $version', tag: 'VERSION_COMPAT');
                  return version;
                }
              }
            }
          ...
          for (final line in lines) {
            if (line.trim().startsWith('MIN_SERVER_VERSION=')) {
              ...
              if (version.isNotEmpty && !version.startsWith('#')) {
                _logger.log('从VERSION文件（旧格式）读取最小服务器版本: $version', tag: 'VERSION_COMPAT');
                return version;
              }
            }
          }
      ...
      // 如果找不到，返回默认值（允许所有版本）
      _logger.log('未找到VERSION.yaml文件，使用默认最小服务器版本: 1.0.0', tag: 'VERSION_COMPAT');
      return '1.0.0';
    } catch (e, stackTrace) {
