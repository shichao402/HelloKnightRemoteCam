...
Future<void> _initializeWindow() async {
  ...
  if (await windowSettings.hasWindowBounds()) {
    await windowManager
        .setSize(Size(savedBounds['width']!, savedBounds['height']!));
    if (savedBounds['x']! > 0 && savedBounds['y']! > 0) {
      await windowManager
          .setPosition(Offset(savedBounds['x']!, savedBounds['y']!));
    } else {
...
Future<void> _checkForUpdateOnStartup(
    UpdateService updateService, ClientLoggerService logger) async {
  try {
